...
Offloading responsibility is not the only motivation for using an agent harness. Harness choice can also affect model usage costs. The [HarnessTax benchmark](https://harnesstax.github.io/) recently reported up to a 5x difference in cost for the same model across Claude Code, Codex CLI, and Pi on SWE-bench Lite and Terminal-Bench 2.0.

Hatchet owns the durable orchestration around the turns. A [durable task](/v1/durable-tasks) running on a worker orchestrates the sequence of turns. It [waits durably](/v1/durable-event-waits) for user-message events and spawns a Pi turn as a [child task](/v1/child-spawning) when one is received. Each child constructs a new Pi session and runs one turn. Hatchet checkpoints the child's result and passes the returned Pi session state into the next turn.
...
<Callout type="warning">
  ...
  <a href="/v1/retry-policies">retry policy</a>.
</Callout>
...
`piSessionManager` is a `hatchet.durableTask` that orchestrates the sequence of turns.

<Snippet
  src={snippets.typescript.pi_agent.manager.define_the_pi_session_manager}
/>

Each iteration begins by waiting for the user message scoped to that turn:
...
<Callout type="info">
  Hatchet may{" "}
  <a href="/v1/durable-tasks#determinism-in-durable-tasks">replay</a> durable
...
A production application should persist enough application state to reconcile completed turns, deduplicate on restarts, and make message delivery safe to repeat.
