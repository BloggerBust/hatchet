...
- Fixes a bug where `hatchet server start` did not honor the active Docker context when `DOCKER_HOST` was unset. The CLI now resolves the Docker host from the active Docker context before falling back to the default Docker behavior.
