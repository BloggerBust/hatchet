...
// The shared e2e worker runs under ts-node with module set to commonjs, where the dynamic
// import in turn.ts compiles to require(), and Pi's package exports only an import
// condition, so that worker cannot load Pi. This test runs its own worker under tsx.
...
// Bound the graceful stop so cleanup cannot hang the suite; SIGKILL follows if the worker
...
async function stopPiWorker(proc: PiWorker | undefined) {
  ...
  proc.kill('SIGTERM');
  const forceKill = setTimeout(() => proc.kill('SIGKILL'), WORKER_STOP_TIMEOUT_MS);
  await exited;
...
describe('pi-agent-e2e', () => {
  ...
  let workerProc: PiWorker | undefined;

  // Runs even when the test times out, so a pending reply iterator cannot leave the worker
  // and its health port behind. The hook timeout exceeds the stop bound so Jest does not
  // abandon the hook before SIGKILL fires.
  afterAll(async () => {
    await stopPiWorker(workerProc);
  }, 2 * WORKER_STOP_TIMEOUT_MS);
  ...
  it(
    ...
    async () => {
      ...
      workerProc = await spawnPiWorker();
